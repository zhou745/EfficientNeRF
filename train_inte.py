...
class NerfTree_Pytorch(object):  # This is only based on Pytorch implementation
    def __init__(self, xyz_min, xyz_max, grid_coarse, grid_fine, deg, sigma_init, sigma_default, device):
        ...
        self.index_voxels_coarse = torch.full((grid_coarse, grid_coarse, grid_coarse), 0, dtype=torch.long,
                                              device=device)
    ...
    def create_voxels_fine(self):
        ...
        self.voxels_fine = torch.zeros(num_valid, self.grid_fine, self.grid_fine, self.grid_fine, self.dim_sh + 1,
                                       device=self.device)
